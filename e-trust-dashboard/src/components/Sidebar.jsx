...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { name: 'Reports / Blacklist', path: '/blacklist', icon: (
    ...
    <aside className="sidebar animate-fade-in">
      ...
      <div className="mt-auto px-2">
        <div className="card" style={{ padding: '0.75rem', background: 'rgba(255,255,255,0.03)', border: 'none', marginBottom: '1rem' }}>
          ...
          <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text-primary)' }}>{user?.storeName || 'Ali Store'}</p>
        </div>
        <button className="btn btn-ghost w-full" onClick={logout} style={{ justifyContent: 'flex-start' }}>
